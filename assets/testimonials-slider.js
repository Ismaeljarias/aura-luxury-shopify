(function() {
  ...
  function initTestimonialsSlider(sectionId) {
    ...
    function calculateItemsPerPage() {
      ...
      while (totalWidth + gap + cardWidth <= wrapperWidth && itemsFit < cards.length) {
    ...
    // Adjust card widths to fit exactly
    ...
    function updateSlider() {
      ...
      // Calculate offset: (cards * cardWidth) + (gaps between cards)
      const itemsToSkip = currentPage * itemsPerPage;
      const gapsToSkip = itemsToSkip; // Same number of gaps as items (gap-right on each card)
      const offset = -(itemsToSkip * cardWidth + gapsToSkip * gap);
    ...
    function recalculate() {
      ...
      if (newItemsPerPage !== itemsPerPage || currentPage === 0) {
